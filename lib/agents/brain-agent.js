...
class BrainAgent {
  ...
  async processMessage(userId, context) {
    ...
    try {
      // Step 1: Analyze user intent with comprehensive understanding
      const userIntent = await this.analyzeIntent(message, imageUrl, sessionState);
      
      // Step 2: Determine appropriate specialized agent
      const targetAgent = await this.determineTargetAgent(userIntent, sessionState);
