...
class ConversationAgent {
  ...
  async gatherBasicInformation(message, session, userIntent) {
    let needsGrade = !session.grade_detected;
    let needsSubject = !session.subject;

    // Check if this message contains grade or subject
    if (needsGrade && userIntent.grade !== "unknown") {
      needsGrade = false;
    ...
    if (needsSubject && userIntent.subject !== "unknown") {
      needsSubject = false;
      await updateSession(session.user_id, {
