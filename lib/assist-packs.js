...
function buildPracticePack(
  ...
  // Determine starting difficulty if not provided
  if (!difficulty) {
    difficulty = initialDifficultyForGrade(grade);
  }
  ...
  footer =
    "Try them. Tell me if you don’t know where to start. Say 'more' for harder. Change topic anytime: e.g. 'stats' or 'geometry'.";
  const text = `${header}:\n${textLines.join("\n")}\n${footer}`;
...
// ---- Grade-aware difficulty helpers ----
function normalizeGradeValue(g) {
  if (!g) return null;
  const n = parseInt(String(g).match(/\d+/)?.[0] || "", 10);
...
// Standard sequence for junior (8-10)
const JUNIOR_SEQUENCE = ["easy", "medium", "hard", "challenge"];
// Senior (11-12) skips medium on first escalation
function gradeAwareNextDifficulty(grade, current) {
  const g = normalizeGradeValue(grade);
  if (!g) return nextDifficulty(current);

  if (g <= 10) {
    const idx = JUNIOR_SEQUENCE.indexOf(current);
    if (idx === -1) return "medium";
    return JUNIOR_SEQUENCE[Math.min(idx + 1, JUNIOR_SEQUENCE.length - 1)];
  }

  // Senior path (11-12)
  if (current === "easy") return "hard";
  if (current === "hard") return "challenge";
  if (current === "medium") return "hard";
  return "challenge";
}

// ---- Diagnostic Single Question Builder ----
function buildDiagnosticQuestion(topicRaw = "algebra", grade = null) {
  const topic = normalizeTopic(topicRaw);
  // Very lightweight topic-based sample questions (can later AI-generate)
  const questionBank = {
    algebra: "Solve for x: 2x + 5 = 17",
    functions: "Given f(x)=x^2−4x, name its turning point.",
    trigonometry: "Solve sinθ = 1/2 for 0° ≤ θ ≤ 180°.",
    statistics: "List the first step to find the median of a data set.",
    geometry: "What is the gradient between (2,3) and (5,9)?",
  };
  const qText = questionBank[topic] || "Start by stating what the question is asking you to find.";
  return {
    help_type: "diagnostic_question",
    topic,
    difficulty: initialDifficultyForGrade ? initialDifficultyForGrade(grade) : "easy",
    question: {
      id: "diag_" + Date.now(),
      text: qText,
    },
    text:
      `Warm-up diagnostic (${topic}).\nQ1) ${qText}\n` +
      `If you don’t know where to start, say 'hint' or 'skip'. Say 'practice' for a full set or 'explain' if you prefer a concept overview.`,
    expectation: "awaiting_answers",
  };
}
